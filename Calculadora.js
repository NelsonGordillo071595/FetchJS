...
function calcular() {
    ...
    email = document.getElementById("input_correo").value;
    ...
    class Consulta {
        constructor(nombre, email, monto, cuotas, tasa, tasa_tipo, periodo, valor){
            this.nombre = nombre
            this.email = email
            this.monto = monto
            this.cuotas= cuotas
    ...
    let dataTemporal = new Consulta(nombre, email, monto, cuotas, tasa, tasa_tipo, periodo, valor)
    ...
    periodo == "mensual" && (msg = nombre + ",usted pagará " + valor + ", mensualmente durante " + items.length + " meses.");
    periodo == "anual" && (msg = nombre + ",usted pagará " + valor + ", anualmente por un periodo de " + items.length + " años");
    ...
    method: 'POST',
    body: JSON.stringify({dataTemporal
    }),
    headers: {
